...
parser.add_argument("--zx",type = int,help = "",default = 4)
...
"""
image_size = 32*zx #args.image_size
...
def main():
  ...
  #data_iter = np.repeat(data_iter[np.newaxis,:],batch_size)
  ...
  fixed_noise = torch.randn(batch_size, nz, 8, 8, device=device)
  ...
  for epoch in range(num_epochs):
    ...
    for i in range(500):
        ...
        if i == 0:
            print("netD input shape:", data.shape,'\n')
        ...
        if (iters % 200 == 0) or ((epoch == num_epochs-1) and (i == len(dataloa)-1)):
            with torch.no_grad():
